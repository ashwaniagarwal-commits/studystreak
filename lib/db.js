// lib/db.js — Postgres adapter for Vercel + Neon.
// Replaces server/db.js (SQLite). Same exported function shape so the API
// routes can use it without caring which backend is underneath.
//
// Connection: @neondatabase/serverless uses an HTTP transport that survives
// across cold starts and avoids socket-pool issues. DATABASE_URL must be set
// (Vercel injects this when you link the Neon integration).
...
/**
 * Map the DB row shape (snake_case) to the engine state shape (camelCase).
 */
export function rowToStreakState(row) {
  ...
  return {
    ...
    longest: row.longest,
    lastStreakDay: row.last_streak_day ? row.last_streak_day.toISOString().slice(0, 10) : null,
