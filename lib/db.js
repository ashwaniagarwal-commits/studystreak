...
export async function ensureSchema() {
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_users_last_active ON users(last_active_at DESC)`;
  await sql`
...
/**
 * Touch the user's last_active_at. Call on every authenticated request.
 * Debounce: only writes if last value is older than 5 minutes (cheap optimisation
 * to avoid hammering the DB on every page-load).
 */
const lastTouchCache = new Map(); // userId → epoch ms
export async function touchActive(userId) {
  if (!userId) return;
  const now = Date.now();
  const last = lastTouchCache.get(userId) || 0;
  if (now - last < 5 * 60 * 1000) return; // <5 min ago, skip
...
/**
 * Counts of active users in 24h / 7d / 30d rolling windows.
 */
...
/**
 * Active-user list with each user's last activity timestamp.
 */
export async function getActiveUsersList(window = '30 days', limit = 200) {
  // window expects '24 hours' / '7 days' / '30 days' — raw-string switch since
  // we can't parameterise an INTERVAL literal via the tagged template.
  if (window === '24 hours') {
    return await sql`
      SELECT u.id, u.display_name, u.batch, u.last_active_at,
             s.current_count AS current_streak, s.total_xp, s.sessions_completed
      FROM users u
...
export async function listAllUsers() {
  return await sql`
    SELECT u.id, u.display_name, u.batch, u.created_at, u.invited_by,
           s.current_count AS current_streak, s.longest, s.total_xp, s.sessions_completed
    FROM users u
    LEFT JOIN streak_state s ON s.user_id = u.id
